...
const FormularioReserva = () => {
    ...
    const handleVueltasChange = (e) => {
        ...
        setReserva({ ...reserva, numeroVueltas: vueltas, duracionTotal: duracion, precioTotal: precioBase * reserva.cantidadPersonas });
    };

    // 🔹 Calcula el precio total al modificar la cantidad de personas
    const handlePersonasChange = (e) => {
        const cantidad = parseInt(e.target.value);
        setReserva({ ...reserva, cantidadPersonas: cantidad, precioTotal: cantidad * (reserva.numeroVueltas === 10 ? 15000 : reserva.numeroVueltas === 15 ? 20000 : 25000) });
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (id) {
            actualizarReserva(id, reserva).then(() => navigate("/reservas"));
        } else {
            crearReserva(reserva).then(() => navigate("/reservas"));
        }
    };
            ...
            <form onSubmit={handleSubmit}>
                <div className="mb-3">
                    ...
                    <DatePicker 
                        ...
                        onChange={(date) => setReserva({ ...reserva, fechaReserva: date })} 
                ...
                <div className="mb-3">
                    ...
                    <input type="time" className="form-control" name="horaInicio" value={reserva.horaInicio} onChange={(e) => setReserva({ ...reserva, horaInicio: e.target.value })} required />
                </div>
                <div className="mb-3">
                    <label className="form-label">Número de Vueltas</label>
                    <select className="form-select" name="numeroVueltas" value={reserva.numeroVueltas} onChange={handleVueltasChange} required>
                ...
                <div className="mb-3">
                    ...
                    <input type="number" className="form-control" name="cantidadPersonas" value={reserva.cantidadPersonas} onChange={handlePersonasChange} required />
                </div>
                <div className="mb-3">
                    <label className="form-label">Día Especial</label>
                    <select className="form-select" name="diaEspecial" value={reserva.diaEspecial} onChange={(e) => setReserva({ ...reserva, diaEspecial: e.target.value })} required>
                ...
                <div className="mb-3">
                    ...
                    <select className="form-select" name="idClienteResponsable" value={reserva.idClienteResponsable} onChange={(e) => setReserva({ ...reserva, idClienteResponsable: e.target.value })} required>
                ...
                <div className="mb-3" key={index}>
                    ...
                    <select className="form-select" onChange={(e) => handleCarroAsignadoChange(index, e.target.value)} required>
                        <option value="">Selecciona un carro</option>
                        {carros.map(carro => (
                            <option key={carro.idCarro} value={carro.codigoCarros}>{carro.codigoCarros}</option>
                        ))}
                    </select>
                    </div>
                ))}
